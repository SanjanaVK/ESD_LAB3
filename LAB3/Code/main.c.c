...
start:do
	...
	while(1)
    {
        ...
        case '+':                //if '+' then add buffer
             ...
             DEBUGPORT(5)          //writes a value given in the external memory if DEBUG is defined
...
void dataout(int x) //a function to execute debug port i.e writing a value in external memory
{
   add= 0xFFFF;
   *add = x;
